...
from employees
         ...
         full outer join role r on r.id = re.role_id
where employee_name isnull
  and salary_id is not null
order by monthly_salary
;

-- можно и сократить конечно..
...
--есть еще ЗП, назначенная сотруднику (известно кому), но без специальности (role_name)
select monthly_salary, employee_name, role_name, salary_id
from employees
         full outer join employee_salary es on employees.id = es.employee_id
         full outer join salary s on s.id = es.salary_id
...
from roles_employee re
         ...
         join role r on r.id = re.employee_id
